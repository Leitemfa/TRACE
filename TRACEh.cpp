...
double one_stage_sampling(arma::mat Y, arma::mat X, double a_alpha, double b_alpha, double prior_var, int max_it, double epsilon, double nmcmc, double burnin, int m, double eps_MH, int truncP){
  int n = Y.n_rows;
...
arma::vec two_stage_sampling(arma::mat Y, arma::mat X, double eta0_rho, double nu0_rho, double gamma0_rho, double lambda0_rho, double prior_var, int max_it, double epsilon, double nmcmc, double burnin, int m, double eps_MH, double alpha_beta){
  int p = Y.n_cols;
  int q = X.n_cols;
...
List MVP_IBPh(arma::mat Y, arma::mat X, double eta0_rho, double nu0_rho, double gamma0_rho, double lambda0_sq_rho, double a_alpha, double b_alpha, double prior_var, int max_it, double epsilon, int m, int nmcmc, int burnin, int nmcmc_a, int burnin_a, double eps_MH, int truncP){
  arma::mat data = Y;
  int n = Y.n_rows;
  Y = arma::join_rows(Y, arma::zeros<arma::mat>(n, truncP));
  int p = Y.n_cols;
  ...
  arma::wall_clock timer;
  timer.tic();
  double alpha_beta = exp(one_stage_sampling(Y, X, a_alpha, b_alpha, prior_var, max_it, epsilon, nmcmc_a, burnin_a, m, eps_MH, truncP));
  arma::vec emp_bayes_estimates = two_stage_sampling(Y, X, eta0_rho, nu0_rho, gamma0_rho, lambda0_sq_rho, prior_var, max_it, epsilon, nmcmc, burnin, m, eps_MH, alpha_beta);
  double alpha1 = emp_bayes_estimates(0);
  double alpha2_sq = emp_bayes_estimates(1);
  arma::mat first_stage = marginal_probit(Y, X, alpha_beta, prior_var, epsilon, max_it);
